...
# ① 四条断言必须以测试形式留在仓里
T=$(grep -rlE '(fun|class) .*(Wheel|Scroll|TwoFinger|Pointer)' \
      app/app/src/test 2>/dev/null | head -3)
[ -n "$T" ] || {
  echo "FAIL 找不到滚轮/双指测试（app/app/src/test 下，名字含 Wheel/Scroll/TwoFinger/Pointer）"
  exit 1
}
echo "  ok   测试文件：$(echo "$T" | tr '\n' ' ')"

miss=""
grep -rqE '双指|twoFinger|TwoFinger|pointerCount|2' $T || miss="$miss R1(双指滑动=滚轮)"
grep -rqE '单指|oneFinger|drag|拖动' $T || miss="$miss R2(单指拖动不退化)"
grep -rqE 'ACTION_SCROLL|generic|物理' $T || miss="$miss R3(物理滚轮不被挡)"
grep -rqE '未开|not.?track|回落|fallback' $T || miss="$miss R4(未开跟踪时仍能滚)"
...
OUT="$ROOT/.team/nodes/t.wheel/tmp/gradle-run.log"
mkdir -p "$(dirname "$OUT")"
...
echo "PASS 四条断言在仓里、壳未自拼 CSI、说明贴了改前的红、服务端与核层零改动（桌面端通路未动）、app 单测真跑绿"
