...
fn default_channel_dir(home: &Path) -> PathBuf {
    home.join(".local/state/nodeprobe/pi-activity")
}

pub fn channel_dir() -> Option<PathBuf> {
    std::env::var_os("NODEPROBE_PI_ACTIVITY_DIR")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| default_channel_dir(Path::new(&home))))
}
...
mod tests {
    ...
    #[test]
    fn default_channel_dir_is_private_per_user_runtime_path() {
        assert_eq!(
            default_channel_dir(Path::new("/Users/example")),
            PathBuf::from("/Users/example/.local/state/nodeprobe/pi-activity")
        );
    }
