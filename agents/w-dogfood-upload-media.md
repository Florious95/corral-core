---
name: w-dogfood-upload-media
role: fix-dogfood-upload-media 承办
provider: codex
...
你承办任务 `fix-dogfood-upload-media`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/fix-dogfood-upload-media/CLAUDE.md`

## 验收（编排引擎会原样复跑，不看你的自报）
- `bash -lc 'env -u TEAM_AGENT_* bash -lc "cd app && ./gradlew -q :app:testDebugUnitTest"'`

## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/fix-dogfood-upload-media.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
