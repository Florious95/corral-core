---
name: w-tsnet-wiring2
role: feat-tsnet-wiring 承办
provider: codex
...
你承办任务 `feat-tsnet-wiring`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/feat-tsnet-wiring/CLAUDE.md`

## 验收（编排引擎会原样复跑，不看你的自报）
- `bash -lc 'cd app && ./gradlew -q :app:testDebugUnitTest'`
- `bash -lc 'cd server && go test ./internal/tsnetd/... ./internal/pairing/...'`

## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/feat-tsnet-wiring.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
