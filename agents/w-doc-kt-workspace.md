---
name: w-doc-kt-workspace
role: doc-contract-kt-workspace 承办（dev.agentmirror.app.workspace 注释刷新与补契约）
...
你承办任务 `doc-contract-kt-workspace`，只负责 **`dev.agentmirror.app.workspace` 这一个包**。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/doc-contract-kt-workspace/CLAUDE.md`
写法手册（必读）：`.team/nodes/arch-criteria-t3/HANDBOOK.md`
任务书原文以 `taskbook.yaml` 的 `doc-contract-kt-workspace` 条目为准。
...
找出属于 `dev.agentmirror.app.workspace` 的那些补上。**只管自己包的**——替别的包补会判红。
只声明真实存在的 import：声明了却没 import 同样判红。

## 验收（leader 会原样复跑，不看你的自报）

- `python3 tools/archwiki/build_wiki.py --check --strict-t3 --pkg dev.agentmirror.app.workspace` —— 单包硬判 exit 0
- 包内测试保持绿
...
- 写入范围严格限于 `app/app/src/main/java/dev/agentmirror/app/workspace/`，越界即退件。
...
`.team/evidence/doc-contract-kt-workspace.json`：`status` 只允许 `pass`/`red`/`blocked`，
