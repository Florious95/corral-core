---
name: w-perf-thresholds-enforce
role: perf-thresholds-enforce 承办
provider: codex
...
你承办任务 `perf-thresholds-enforce`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/perf-thresholds-enforce/CLAUDE.md`
...
## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/perf-thresholds-enforce.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
