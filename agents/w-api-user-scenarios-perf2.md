---
name: w-api-user-scenarios-perf2
role: test-api-user-scenarios-perf 承办
provider: codex
...
你承办任务 `test-api-user-scenarios-perf`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/test-api-user-scenarios-perf/CLAUDE.md`
...
## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/test-api-user-scenarios-perf.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
