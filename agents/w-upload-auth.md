...
你承办任务 `fix-upload-auth`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/fix-upload-auth/CLAUDE.md`
...
## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/fix-upload-auth.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
