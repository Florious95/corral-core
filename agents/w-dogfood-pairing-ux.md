---
name: w-dogfood-pairing-ux
role: fix-dogfood-pairing-ux 承办
provider: codex
...
你承办任务 `fix-dogfood-pairing-ux`。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/fix-dogfood-pairing-ux/CLAUDE.md`
...
## 交件契约（三个值一字不改，源码级约束）
1. **先把证据写盘** `.team/evidence/fix-dogfood-pairing-ux.json`：`status` 只允许 `pass`/`red`/`blocked` 三值，
