---
name: w-doc-agentstate
role: doc-contract-agentstate 承办（internal/agentstate 注释刷新与补契约）
...
你承办任务 `doc-contract-agentstate`，只负责 **`internal/agentstate` 这一个包**。**一次性席位，交件即退役。**
知识基底（开工前完整读）：`.team/nodes/doc-contract-agentstate/CLAUDE.md`
写法手册（必读）：`.team/nodes/arch-criteria-t3/HANDBOOK.md`
任务书原文以 `taskbook.yaml` 的 `doc-contract-agentstate` 条目为准。
...
找出属于 `internal/agentstate` 的那些，补 `@consumes` 声明使之与真实 import 图一致。
只声明真实存在的 import——声明了却没 import 同样判红。

## 验收（leader 会原样复跑，不看你的自报）

- `python3 tools/archwiki/build_wiki.py --check --strict-t3 --pkg internal/agentstate` —— 单包硬判必须 exit 0
- 包内测试必须保持绿
- `python3 tools/archwiki/build_wiki.py --check` —— 全仓库既有判据不许被打坏

**阳性对照要求**：不许只看 rc=0。收工前**故意破坏一次**——把某个符号的 `@err` 删掉，
确认 `--strict-t3 --pkg internal/agentstate` 真的 exit 1 并指向该符号，然后改回来。
...
- 写入范围严格限于 `server/internal/agentstate/`，越界即退件。
...
`.team/evidence/doc-contract-agentstate.json`：`status` 只允许 `pass`/`red`/`blocked`，
