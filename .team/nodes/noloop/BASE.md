# 知识基底 · ledger.v74.v1 / t.noloop（tools/basegen_ledger.py 编译产物，手工编辑无效）

## 1. 任务信封（账本原文，机械抽取）
```
修 **App 无限刷新**。契约 requirement-base/entries/074-无限刷新与未知回归收尾.md §1。
用户实测：二级菜单列表中央**转圈不停**，持续处于刷新态。
🔴 **先把环路量出来再改**：记一次进入菜单后 10 秒内的 `list` 请求数与 `level2_frame` 推送数，**两个数都要有**。⛔ 不要照 leader 那条猜测（list→推送→list 自激）直接动手——那只是方向，不是结论。
判据 `A-rf-noloop`：进入二级菜单后静置 10 秒，`list` 请求数 **≤ 2** 且**不随时间线性增长**；转圈指示在**首帧到达后必须消失**。🔴 先在当前实现上验红，把红数据（两个计数）写进说明。
🔴 **不得倒退 069 的「进菜单即时刷新」**——修法不是把即时刷新关掉，而是**断掉自激**（推送不得再触发 list）。用户明确说过刷新及时这一点手感很好。
配套测试名须含 `NoRefreshLoop`。说明写 /Volumes/nvme/Projects/远程Agent安卓/.team/nodes/noloop/说明.md。
...
- write_paths: app/, server/, .team/nodes/noloop/
- read_paths: requirement-base/entries/074-无限刷新与未知回归收尾.md, requirement-base/entries/069-进菜单必触发一次即时刷新.md, app/app/src/, server/internal/
- 判据: A-nl-test, A-nl-counts, A-nl-go, A-nl-app
...
## 3. 需求基
- 标题引用条目：requirement-base/entries/074*
