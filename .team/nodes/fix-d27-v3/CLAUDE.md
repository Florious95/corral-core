# 知识基底 · fix-d27-v3（tools/basegen.py 编译产物——手工编辑无效，重编请改素材源后重跑）

## 1. 任务信封（taskbook.yaml 原文，机械抽取）
```yaml
  - id: fix-d27-v3
    goal: >
      D-27 第二次回炉：发消息后逐行刷新。核心问题——为什么发消息追加和 CLI 自己吐字追加
      表现不同？两者本质都是 CLI 底部追加内容。找到差异点，统一实现。
    acceptance:
    ...
    evidence: ".team/evidence/fix-d27-v3.json"
    contention: none
```
