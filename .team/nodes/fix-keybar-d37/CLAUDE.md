# 知识基底 · fix-keybar-d37（tools/basegen.py 编译产物——手工编辑无效，重编请改素材源后重跑）

## 1. 任务信封（taskbook.yaml 原文，机械抽取）
```yaml
  - id: fix-keybar-d37
    goal: >
      D-37 特殊键条不支持连按。MVP 方案已出（.team/evidence/research-keybar-rapid.md）：
    ...
    evidence: ".team/evidence/fix-keybar-d37.json"
    contention: none
```
