# 知识基底 · ledger.hl1.v1 / t.keep（tools/basegen_ledger.py 编译产物，手工编辑无效）

## 1. 任务信封（账本原文，机械抽取）
```
# t.keep · 连接保活与半开自愈（契约 100，分支 pr/conn-keepalive）

用户实测：换服务端后手机 app「无限重连中」——横幅在转但从不真正重拨（半开连接）。
审计确认（audit-20260821-opus-findings.json lifecycle 维度）：WebSocket 无任何保活。
1. 先验红：单测钉「显示重连中但无活跃 dial」这个状态可达（模拟半开：socket 不关但对端死）——当前 main 红。
2. 修法（契约 100 四条）：应用层 ping/pong 保活+超时判死强制重拨；重连状态机保证显示重连中 ⇔ 存在 dial+退避；onResume 主动 probe。服务端 pong 配合（先核协议现状）。
3. 全量单测不倒退。
交付 .team/nodes/hl1-keep/说明.md：status=done、根因=、修法=、先验红输出=、转绿输出=、服务端配合=。
纪律：分支 pr/conn-keepalive；write_paths 含 app/ 与 server/（若需 pong）；⛔ 不碰 main、不 push；临时文件只写 .team/nodes/hl1-keep/tmp/；如实报不可判是合法出口。

```

- write_paths: app/, server/, .team/nodes/hl1-keep/
- read_paths: requirement-base/entries/100-连接保活与半开重连.md, .team/artifacts/audit-20260821-opus-findings.json
- 判据: A-keep-suite, A-keep-go, A-keep-doc
