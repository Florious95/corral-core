...
provenance_revision = "0df3562b7f7479ce4a2683f8c98546fab69bcf1c"
...
continuity = Task(
    title="按 .team/nodes/spec-sol/baseline-bundle-successor7/final-continuity-任务书.md 在 retained wt-maple-core 只读复核 3528 四格、da46 bootstrap、0df 连续性与双归档身份；本格是 command，不启动设备。",
    owner_role="continuity",
    seat_wait_seconds=300,
    parallel="successor7-apparatus-wave",
    resources=resources(
        core_wt,
        [],
        [
...
apparatus_test = Task(
    title="按 .team/nodes/spec-sol/baseline-bundle-successor7/test-任务书.md 在全新隔离 WT 重跑 repo-local fake apparatus/permanent-fixture/structure 齿；严禁 adb、qemu、emulator，只写 RED.md，只 report_result 一次。",
    owner_role="test",
    ...
    resources=resources(
        ...
        [
            ...
            ".team/ledgers/acceptance/baseline-bundle-successor7-emulator-regression.sh",
            ".team/ledgers/acceptance/baseline-bundle-successor7-bypass-regression.sh",
            ".team/ledgers/acceptance/fixtures/baseline-bundle-successor7/",
        ],
    ),
    handoff=Handoff(required_artifacts=[".team/nodes/baseline-bundle-successor7-test/RED.md"]),
    checks=[check("M.baseline-bundle.successor7-test", ".team/ledgers/acceptance/baseline-bundle-successor7-test.sh", 600)],
)

apparatus_probe = Task(
    title="按 .team/nodes/spec-sol/baseline-bundle-successor7/probe-任务书.md 在全新隔离 WT 独立复核有界 ownership、0600、permanent fixture 与九格结构；严禁启动 adb/qemu/emulator，只写 PROBE.md，只 report_result 一次。",
    owner_role="probe",
    seat_wait_seconds=3600,
    parallel="successor7-apparatus-wave",
    resources=resources(
    ...
    handoff=Handoff(required_artifacts=[".team/nodes/baseline-bundle-successor7-probe/PROBE.md"]),
    checks=[check("M.baseline-bundle.successor7-probe", ".team/ledgers/acceptance/baseline-bundle-successor7-probe.sh", 600)],
)

apparatus = Task(
    title="精确 command 格：cwd=retained wt-maple-core，argv=/bin/sh .team/ledgers/acceptance/baseline-bundle-successor7-owned-emulator.sh。三个上游成功后才允许执行，产出0600 APPARATUS.json；按 .team/nodes/spec-sol/baseline-bundle-successor7/apparatus-任务书.md 的所有有界 ownership/清理契约。",
    ...
    executor="command",
    command=CommandSpec(
        argv=["/bin/sh", ".team/ledgers/acceptance/baseline-bundle-successor7-owned-emulator.sh"],
        cwd="${worktree}",
        expected_exit_code=0,
        unjudgeable_exit_codes=[2],
    ...
    handoff=Handoff(required_artifacts=[".team/nodes/baseline-bundle-apparatus/APPARATUS.json"]),
    checks=[
...
ledger = Ledger(
    ledger_id="ledger.baseline-bundle.successor7.v1",
    roles=roles,
    tasks={
        "t.baseline-bundle.continuity": continuity,
        "t.baseline-bundle.apparatus-test": apparatus_test,
        "t.baseline-bundle.apparatus-probe": apparatus_probe,
    ...
    dependencies=[
        dep("t.baseline-bundle.continuity", "t.baseline-bundle.apparatus"),
        dep("t.baseline-bundle.apparatus-test", "t.baseline-bundle.apparatus"),
        dep("t.baseline-bundle.apparatus-probe", "t.baseline-bundle.apparatus"),
    ...
    parallelism=[{"group": "successor7-apparatus-wave", "max_concurrency": 3, "failure_policy": "halt"}],
    fallback={
