...
provenance_revision = "e6c2e2625bb5ce463282b13b2949b3538a2dbbbe"
...
continuity_consume = Task(
    title="审计消费 successor8 r4 continuity succeeded：在原 wt-maple-core 精确重跑现有 successor7 continuity required；绑定61af5e3c4/6dbf110a5/bd48271b9/0fdee1072/e6c2e2625、r4 ledger+driver 与 command-pair=7485102b26ed34eb828e94900902147d5e00e995，不 collect/重派/起设备。",
    owner_role="continuity",
    seat_wait_seconds=300,
    parallel="successor9-consume-wave",
    resources=resources(
        core_wt,
        [],
        [
...
apparatus_test_consume = Task(
    title="审计消费 successor8 r4 apparatus-test succeeded：在原 wt-s7-cedar 精确重跑 successor7-test required，只读消费 SHA=04cdbd661548a4b3261c88d491cf80c48f98dcbe3c080e710fb7d12bbe6c105a 的 RED.md；绑定61af5e3c4 r4 ledger+driver 与 command-pair=7485102b26ed34eb828e94900902147d5e00e995，不发 agent。",
    owner_role="test",
...
apparatus_probe_consume = Task(
    title="审计消费 successor8 r4 apparatus-probe succeeded：在原 wt-s7-orbit 精确重跑 successor7-probe required，只读消费 SHA=88868a1a1979d3f1504e5efd6876dc5ca8ed5cc6b45a2eb6f6dd23b8e5176cf7 的 PROBE.md；绑定61af5e3c4 r4 ledger+driver 与 command-pair=7485102b26ed34eb828e94900902147d5e00e995，不发 agent。",
    owner_role="probe",
    seat_wait_seconds=3600,
    parallel="successor9-consume-wave",
    resources=resources(
...
apparatus = Task(
    title="精确 successor9 apparatus command：cwd=retained wt-maple-core，argv=/bin/sh .team/ledgers/acceptance/baseline-bundle-successor9-owned-emulator.sh；三 consume 成功后才运行。先 strict envcheck，再唯一 SDK selector，再原 successor7 owned-emulator；产出0600 APPARATUS.json，完整保留有界 PID+serial ownership/只清 owned qemu/恢复闸。",
...
ledger = Ledger(
    ledger_id="ledger.baseline-bundle.successor9.v1",
    roles=roles,
    tasks={
    ...
    parallelism=[{"group": "successor9-consume-wave", "max_concurrency": 3, "failure_policy": "halt"}],
    fallback={
