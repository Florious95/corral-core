#!/bin/sh
# //! purpose: 严格合取 successor6 bundle、用户 gate、旧链迁移和 fresh 性能门的独立终审。
# //! contract: 0=根治链完成；1=有效反证；2=环境或证据不可判。
# ledger: expected_exit_code=0; unjudgeable_exit_codes=[2]

set -u
fail() { printf '%s\n' "FAIL baseline-bundle-successor6-final: $*" >&2; exit 1; }
unjudgeable() { printf '%s\n' "UNJUDGEABLE baseline-bundle-successor6-final: $*" >&2; exit 2; }
...
for gate in baseline-bundle-successor6-verify.sh baseline-bundle-user-gate.sh baseline-bundle-migrate.sh baseline-bundle-successor3-measure.sh baseline-bundle-successor3-bypass.sh; do
    sh "$script_dir/$gate"
    rc=$?
...
printf '%s\n' "PASS baseline-bundle-successor6-final: immutable recoverable baseline, migration, fresh lab gate and user gold all pass"
