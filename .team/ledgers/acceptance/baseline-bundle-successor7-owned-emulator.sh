#!/bin/sh
# //! purpose: 有限时创建 repo-local fresh AVD，委托已验 ownership runner 绑定唯一 qemu+serial，安装 retained bundle，清理后留机器证据。
# //! contract: 0=preflight/measurement/install/owned cleanup/recovery 全绿；1=固定身份或证据矛盾；2=环境、设备、超时或清理不可判。
# //! boundary: production 无可替换 root/runner；fixture override 只能写 spec-sol node-local temp，不能生成官方证据。
# ledger: expected_exit_code=0; unjudgeable_exit_codes=[2]

set -u
umask 077

fail() { printf '%s\n' "FAIL baseline-bundle-successor7-owned-emulator: $*" >&2; exit 1; }
unjudgeable() { printf '%s\n' "UNJUDGEABLE baseline-bundle-successor7-owned-emulator: $*" >&2; exit 2; }
...
test_override_count=$(env | awk -F= '/^SUCCESSOR7_TEST_[A-Za-z0-9_]*=/{count++} END{print count+0}')
fixture_selector=${SUCCESSOR7_FIXTURE_MODE:-}
...
if [ "$mode" = fixture ]; then
    [ "${SUCCESSOR7_TEST_HARNESS:-}" = baseline-bundle-successor7-emulator-regression ] || fail "explicit isolated test harness marker missing"
    test_override_names=$(env | sed -n 's/^\(SUCCESSOR7_TEST_[A-Za-z0-9_]*\)=.*/\1/p')
    for test_name in $test_override_names; do
        case "$test_name" in
        SUCCESSOR7_TEST_HARNESS|SUCCESSOR7_TEST_FIXTURE_ROOT|SUCCESSOR7_TEST_ENVCHECK|SUCCESSOR7_TEST_RUNNER|SUCCESSOR7_TEST_CONTINUITY|SUCCESSOR7_TEST_SDK_ROOT|SUCCESSOR7_TEST_GATE_TIMEOUT|SUCCESSOR7_TEST_AVD_TIMEOUT|SUCCESSOR7_TEST_READY_TIMEOUT|SUCCESSOR7_TEST_ADB_TIMEOUT|SUCCESSOR7_TEST_RECOVERY_TIMEOUT|SUCCESSOR7_TEST_TERM_GRACE|SUCCESSOR7_TEST_KILL_GRACE) ;;
        *) fail "unknown test override in isolated mode" ;;
        esac
    done
    fixture_root=${SUCCESSOR7_TEST_FIXTURE_ROOT:-}
    [ -n "$fixture_root" ] || unjudgeable "fixture root missing"
    case "$fixture_root" in "$repo_root/.team/nodes/spec-sol/baseline-bundle-successor7/tmp/"*) ;; *) fail "fixture root escapes node-local temp" ;; esac
    envcheck=${SUCCESSOR7_TEST_ENVCHECK:-}
    runner=${SUCCESSOR7_TEST_RUNNER:-}
    continuity=${SUCCESSOR7_TEST_CONTINUITY:-}
    sdk_root=${SUCCESSOR7_TEST_SDK_ROOT:-}
    evidence="$fixture_root/APPARATUS.json"
    run_root="$fixture_root/run"
    gate_timeout=${SUCCESSOR7_TEST_GATE_TIMEOUT:-3}
    avd_timeout=${SUCCESSOR7_TEST_AVD_TIMEOUT:-5}
    ready_timeout=${SUCCESSOR7_TEST_READY_TIMEOUT:-5}
    adb_timeout=${SUCCESSOR7_TEST_ADB_TIMEOUT:-3}
    recovery_timeout=${SUCCESSOR7_TEST_RECOVERY_TIMEOUT:-3}
    term_grace=${SUCCESSOR7_TEST_TERM_GRACE:-2}
    kill_grace=${SUCCESSOR7_TEST_KILL_GRACE:-2}
else
...
for item in "$helper" "$continuity" "$envcheck" "$runner"; do [ -r "$item" ] && [ -s "$item" ] || unjudgeable "required apparatus input unavailable"; done
...
cleanup_tmp() {
    case "$run_root" in
        "$repo_root/.team/nodes/baseline-bundle-apparatus/tmp/"*|"$repo_root/.team/nodes/spec-sol/baseline-bundle-successor7/tmp/"*) rm -rf "$run_root" ;;
        *) return 2 ;;
    esac
}
...
if [ "$mode" = production ]; then
    sdk_gate="$script_dir/baseline-bundle-successor5-sdk.sh"
    [ -r "$sdk_gate" ] && [ -s "$sdk_gate" ] || abort_run "SDK gate unavailable"
    if ! run_bounded 60 /dev/null sh "$sdk_gate"; then
        abort_run "SDK fallback unavailable"
    fi
...
mkdir -p "$avd_home" || abort_run "cannot create task-owned AVD home"
printf '%s\n' no >"$avd_input" || abort_run "cannot prepare bounded avdmanager input"
run_bounded "$avd_timeout" /dev/null env ANDROID_AVD_HOME="$avd_home" "$avdmanager" create avd --force --name successor7_verify_owned --package "$system_image" <"$avd_input"
avd_rc=$?
[ "$avd_rc" -eq 0 ] || abort_run "fresh AVD creation rc=$avd_rc"
[ -d "$avd_home/successor7_verify_owned.avd" ] || abort_run "fresh AVD directory missing"
...
env ANDROID_AVD_HOME="$avd_home" RUNNER_EVENT_LOG="$event_log" ENV_CHECK="$envcheck" ADB="$adb" \
    EMULATOR_LAUNCHER="$emulator" EMULATOR_AVD=successor7_verify_owned EMULATOR_SERIAL=emulator-5554 \
...
printf '%s\n' "PASS baseline-bundle-successor7-owned-emulator: fresh owned emulator installed retained bundle and was fully reaped"
