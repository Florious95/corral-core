...
set -u
fail() { printf '%s\n' "FAIL baseline-bundle-verify: $*" >&2; exit 1; }
unjudgeable() { printf '%s\n' "UNJUDGEABLE baseline-bundle-verify: $*" >&2; exit 2; }
...
sh "$script_dir/baseline-bundle-impl.sh"
rc=$?
...
except Exception as exc: print("UNJUDGEABLE baseline-bundle-verify:",exc,file=sys.stderr); sys.exit(2)
...
if bad: print("FAIL baseline-bundle-verify: "+", ".join(bad),file=sys.stderr); sys.exit(1)
print("verify-json-ok")
PY
case "$?" in 0) ;; 1) exit 1 ;; 2) exit 2 ;; *) unjudgeable "verify json unsupported rc" ;; esac
printf '%s\n' "PASS baseline-bundle-verify: independent restore/install/mutation gate passed"
