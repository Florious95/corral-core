#!/bin/sh
# //! purpose: 从固定仓根独立重算真实 Baseline Bundle 的来源、APK、运行内容、工具与双份归档。
# //! contract: 0=真实 bundle 全部相符；1=实现/证据被反证；2=环境或必要外部量具不可判。
# ledger: expected_exit_code=0; unjudgeable_exit_codes=[2]

set -u
fail() { printf '%s\n' "FAIL baseline-bundle-impl: $*" >&2; exit 1; }
unjudgeable() { printf '%s\n' "UNJUDGEABLE baseline-bundle-impl: $*" >&2; exit 2; }
...
def fail(msg): print("FAIL baseline-bundle-impl: "+msg,file=sys.stderr); raise SystemExit(1)
def uj(msg): print("UNJUDGEABLE baseline-bundle-impl: "+msg,file=sys.stderr); raise SystemExit(2)
def sha(path):
    h=hashlib.sha256()
    try:
...
if route=="recover_exact_artifact":
    ...
    for i,b in enumerate(builds,1):
        ...
        p=safe_rel(need(b,"apk_relpath",str),f".team/private/baseline-vault/{bid}/builds/")
...
printf '%s\n' "PASS baseline-bundle-impl: fixed-provenance real bundle independently reconstructed and archived"
