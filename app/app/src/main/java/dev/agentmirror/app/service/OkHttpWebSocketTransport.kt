...
object OkHttpTransportFactory : TransportFactory {
    ...
    internal fun create(url: String, recordConnectionPath: Boolean): WebSocketTransport {
        ...
        // sf != null（SOCKS 隧道）必须 .proxy(Proxy.NO_PROXY)：OkHttp 默认继承系统
        // ProxySelector（Clash/Shadowrocket），会把 tailnet 目标先路由到系统代理
        // （127.0.0.1:7892 等），而该地址在 tsnet netstack 内不存在 → SOCKS dial ok 但
        // 握手 unexpected EOF → 死循环（缺陷⑤ 真根因，用户真机日志坐实）。上传器同款
        // 已在 HttpUrlConnectionUploader.kt:174 修，这里只是对齐。sf==null（LAN/域名/未
        // Up 直拨）分支不动——强制代理网络下直连需要系统代理出去，加 NO_PROXY 会误伤
        // （SystemProxyLeakProbeTest T2 防过度修复）。判据=「是否自建 SOCKS 隧道」，
        // 由 sf!=null 分叉的代码形状保证，不散到别处。
        val chosen = if (sf == null) {
            client
        } else {
            client.newBuilder().socketFactory(sf).proxy(Proxy.NO_PROXY).build()
        }
