 ...
 *
 * 动画决策：进入 fade+上滑 6dp、旧行立即移除，180ms CSS ease——
 * 逐项对应 HTML 版 rowIn 动效。三个态叠在同一
...
/** Constant-height source row that switches among favorites, menu, and hotkeys. */
@Composable
fun DockSecondRow(
    mode: DockRowMode,
    ...
    val rowRisePx = with(LocalDensity.current) { 6.dp.roundToPx() }
    val enter = fadeIn(
        animationSpec = tween(
            durationMillis = SessionDockMotion.RowInMillis,
        ...
        initialOffsetY = { rowRisePx },
    )
    // The source removes the previous conditional row immediately; only the new row runs rowIn.
