...
class PairingViewModelTest {
    ...
    fun scanValidQrPairsAndPersists() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"ABC123","ts_authkey":""}""")
        // 进入试配对：配置已携带 URL+token。
        assertEquals("ws://192.0.2.10:9900/ws", h.nextConfig?.url)
        assertEquals("ABC123", h.nextConfig?.token)
        // auth 通过 → 成功 + 持久化。
        h.authOk()
        assertEquals(PairingStatus.Success, h.vm.pairingStatus)
        assertEquals("ws://192.0.2.10:9900/ws", h.store.saved?.url)
    ...
    fun scanRejectsMissingToken() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws"}""")
        val st = h.vm.pairingStatus
        assertTrue(st is PairingStatus.Failed)
    ...
    fun scanRejectsBadVersion() {
        val h = Harness()
        h.vm.onQrText("""{"v":2,"url":"ws://192.0.2.10:9900/ws","token":"ABC123"}""")
        val st = h.vm.pairingStatus
        assertTrue(st is PairingStatus.Failed)
    ...
    fun scanRejectsInvalidUrl() {
        ...
        h.vm.onQrText("""{"v":1,"url":"http://192.0.2.10:9900/ws","token":"ABC123"}""")
        val st = h.vm.pairingStatus
        assertTrue(st is PairingStatus.Failed)
    ...
    fun scanWithTsAuthKeyStartsTsnetAndPersistsKey() {
        ...
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"ABC123","ts_authkey":"tskey-qr-1"}""")
    ...
    fun manualTsAuthKeyStartsTsnetOnSubmit() {
        val started = mutableListOf<String>()
        val h = Harness(tsnetStarter = { started.add(it) })
        h.vm.manualUrl = "ws://192.0.2.10:9900/ws"
    ...
    fun secureStoreFailureBecomesFixedVisibleError() {
        ...
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"ABC123"}""")
        h.authOk()
    ...
    fun scanDialFailureSurfacesImmediatelyNotSilent() {
        val h = Harness()
        h.dialFails()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"ABC"}""")
    ...
    fun scanImmediatelyAutoConnectsWithTargetUrlVisible() {
        ...
        assertEquals("ws://192.0.2.10:9900/ws", (st as PairingStatus.Pairing).targetUrl)
    }
    ...
    fun failThenRetryReconnectsWithSameConfig() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"ABC"}""")
        assertEquals(1, h.identifyCalls)
        h.authReject()
        assertEquals(PairingFailCause.REJECTED, h.failedCause())
        // 失败态重试必须先重新完成身份验证，再创建第二个 WebSocket 探针。
        assertTrue(h.vm.canRetry)
        h.vm.retry()
        assertEquals(2, h.identifyCalls)
        val st = h.vm.pairingStatus
        assertTrue("expected Pairing, got $st", st is PairingStatus.Pairing)
        assertEquals("ws://192.0.2.10:9900/ws", h.nextConfig?.url)
    ...
    fun authFailureSurfacesExplicitRejection() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"WRONG"}""")
    ...
    fun pairTimeoutSurfacesExplicitFailure() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"SLOW"}""")
    ...
    fun resetReturnsToIdleWithoutReject() {
        val h = Harness()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"X"}""")
        // 配对中 reset：不得误报"拒绝"（旧探针 stop 的 STOPPED 回调被短路）。
        h.vm.reset()
        assertEquals(PairingStatus.Idle, h.vm.pairingStatus)
        // 可立即重新配对。
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"Y"}""")
        h.authOk()
        assertEquals(PairingStatus.Success, h.vm.pairingStatus)
    ...
    fun errorMessageNeverContainsTokenValue() {
        ...
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"$secret"}""")
        h.authReject("token_mismatch")
    ...
    fun scanFailsPrimaryThenAutoTriesCandidatesSuccess() {
        ...
        h.dialFails()
        h.vm.onQrText(
            """{"v":1,"url":"ws://192.0.2.11:1/ws","token":"T","ts_authkey":"","candidates":["ws://192.0.2.11:1/ws","ws://192.0.2.12:1/ws"]}""",
        )
        // 拨号失败→推进逐试全在 onQrText 内同步完成：返回时已自动推进到第二候选 good 且拨号成功。
        assertEquals(2, h.transports.size)
        assertEquals("ws://192.0.2.12:1/ws", h.nextConfig?.url)
        assertEquals("T", h.nextConfig?.token)
        // 候选 good：拨号成功 → auth_ack ok → 配对成功（FIELD.md 模拟器走查：错误主选+正确候选）。
        h.authOk()
        assertEquals(PairingStatus.Success, h.vm.pairingStatus)
        assertEquals("ws://192.0.2.12:1/ws", h.store.saved?.url)
    ...
    fun scanFailsPrimaryTimeoutAdvancesToNextCandidate() {
        val h = Harness()
        h.vm.onQrText(
            """{"v":1,"url":"ws://192.0.2.13:1/ws","token":"T","candidates":["ws://192.0.2.13:1/ws","ws://192.0.2.14:1/ws"]}""",
        )
        // 主选拨号成功但不回 auth → 超时（候选预算 3s）→ 自动推进逐试。
        h.clock.advance(3_001L)
        h.vm.onTick(h.clock.nowMs())
        assertEquals(2, h.transports.size)
        assertEquals("ws://192.0.2.14:1/ws", h.nextConfig?.url)
    }
    ...
    fun allCandidatesFailShowsFullCandidateListAndCanRetryCandidate() {
        val h = Harness()
        h.alwaysDialFails()
        val candidates = """["ws://192.0.2.15:1/ws","ws://192.0.2.16:1/ws"]"""
        h.vm.onQrText(
            """{"v":1,"url":"ws://192.0.2.15:1/ws","token":"T","ts_authkey":"","candidates":$candidates}""",
        )
        // 逐试完两个候选（主选 a + 候选 b）全败 → 落最终失败态 + 候选列表可见。
        assertEquals(listOf("ws://192.0.2.15:1/ws", "ws://192.0.2.16:1/ws"), h.vm.candidateUrls)
        val st = h.vm.pairingStatus
        assertTrue("expected Failed, got $st", st is PairingStatus.Failed)
        ...
        h.failingDial = false
        h.vm.retryCandidate("ws://192.0.2.16:1/ws")
        assertTrue(h.vm.pairingStatus is PairingStatus.Pairing)
        assertEquals("ws://192.0.2.16:1/ws", h.nextConfig?.url)
        assertEquals(listOf("ws://192.0.2.15:1/ws", "ws://192.0.2.16:1/ws"), h.vm.candidateUrls)
    }
    ...
    fun failureClearsInProgressRecognitionText() {
        ...
        val h = Harness()
        h.dialFails()
        h.vm.onQrText("""{"v":1,"url":"ws://192.0.2.10:9900/ws","token":"T"}""")
    ...
    fun scanCandidatesDeduplicatedWithPrimary() {
        ...
        h.alwaysDialFails()
        h.vm.onQrText(
            """{"v":1,"url":"ws://192.0.2.15:1/ws","token":"T","candidates":["ws://192.0.2.15:1/ws","ws://192.0.2.16:1/ws","ws://192.0.2.15:1/ws"]}""",
        )
        assertEquals(2, h.transports.size)
        assertEquals(listOf("ws://192.0.2.15:1/ws", "ws://192.0.2.16:1/ws"), h.vm.candidateUrls)
    }
