...
//
// 背景（leader msg_22b66343e7b8）：sendMirror 非阻塞发 sendCh，队列满时不能继续
// 伪装成完整流；服务端立即中止连接，由客户端现有重连/重订快照对账。慢链路（TS）
// 队列常满 → 连接中止 → 客户端 replaySnapshot 清屏重建；局域网队列从不满 → 不丢 → 流畅。
// DeltasDropped 仍只计 WS sendCh 满事件；bridge 原始流 loss 通过连接关闭原因观测。
//
