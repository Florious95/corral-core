...
## 8. 【修正】§4 收窄「未知」的写法被实测证伪（2026-08-19 用户报）

§4 原文：「非白名单 pane 不进列表；**未知只保留给：已确定是哪一家，但该家的检测器认不出这串标题**」。

🔴 **后半句盖掉了 062 的三态规则，是回归。** 用户实测：
> 新对话被标记为未知

现场量到的根因：**刚起的会话，pane 标题就是一个光秃秃的产品名**（不带该家的空闲后缀），
本家检测器不认领 ⇒ 按 §4 判成「未知」。而 062 明确规定 **无前导符号（字母/数字/空）⇒ 空闲**。

**修正后的语义（以 062 为准）**：

1. provider 由 comm 确定（§2 不变，身份优先不变——只跑本家检测器，⛔ 不让别家认领）；
2. 本家检测器认领 ⇒ 用它的结论；
3. **不认领 ⇒ 落回 062 三态**：无前导符号 ⇒ **空闲**；有前导符号但认不出 ⇒ **未知**。

⇒ **未知只由「认不出的前导符号」产生**。那才是「某家判据缺样本」这条真信号；
把「标题没长成我预期的样子」也算未知，会让每个新开的会话都先红一下。

**判据**：语料 `tools/nodeprobe/fixtures/titles.tsv` 新增一行——
裸产品名标题 ⇒ `idle`。🔴 此条已先在旧实现上验红（`go="unknown" want="idle"`）。
