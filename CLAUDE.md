...
🔴 **2026-08-24 用户再次裁定：不得因为全自动编排框架的阻塞而阻塞产品任务。发现缺陷并把报告投递后，框架责任即完成交割；框架修复、回复或新基础设施均不得成为产品继续推进的前置条件。**「继续」不是保持 park、等待通知或只做旁支审查，而是立即沿当前任务的既定需求、任务书、判据和证据约束推进下一项可执行工作；仍禁止替框架队调查或私改其 skill/引擎。

- 用户原话（2026-08-18）：「如果框架停下来，你不用停下来。我们来日方长，使用这个框架的机会还很多。
  你把报告投递了，就可以让任务继续了。」
- 用户再次原话（2026-08-24）：「不要因为全自动编排框架的阻塞而阻塞你的任务。你只要发现了任务投递了之后，就可以继续了。」
