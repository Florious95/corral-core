...
🔴 **禁止 leader 做大量消耗上下文的操作**（2026-08-24 用户令）：通读大树、反复 grep/读大文件、
把整份日志/案卷/截图流塞进自己会话——一律派给 TeamMate。这类活必须**写成账本格**，
由 `ledger-run` 派出、嵌入全自动编排；⛔ 不要 leader 亲自干，也不要用人工 send 顶替立格。
leader 只留判据、裁定、并线。
