...
> **⑤ 新增的观测仪表，第一件事是验证它测的是不是你以为的那个东西。**
> 仪表的作用域（单连接/单会话/进程/全局）必须在字段名或日志里显式标出。
> 依据：本轮 `snapshots_pushed` 为进程级累计却打在 per-conn 的 teardown 行上，
> 被读成单连接数值，据此产生「注延迟下快照暴增」的错误线索并追查数轮。
> 仪表的正确性和被测代码的正确性同等重要，而且更容易被忽略——仪表出错时，
> 它输出的仍然是数字，看起来仍然像证据。
...
> **⑥ 用模拟物代替被测对象，验证的是「如果 X 成立会怎样」，不是「X 成不成立」。**
> 「我们验的场景和用户用的场景不是同一条路」是本工程反复出现的形状：
> 用 bash 测「发消息整屏刷」，而用户用的是 Claude Code；在 1080×2400 上测几何缺陷，
> 而用户是 1260×2800；用内容不足的终端量「底部空白」。
> 本轮实例：librarian 测「秒数自涨会不会造成内容对比假变化」，用 `printf "\rBrewed %s" $(date +%s)`
> 每秒刷新——**那是自己造的一个每秒刷新的东西**，结论是构造出来的。真实要问的是
> 「真实 Claude Code 完成后的 `✳ Brewed for 42m 3s` 秒数会不会自己涨」（它是一句话，不是跳动的时钟）。
