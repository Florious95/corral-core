...
## 五、存活判据修订（leader 裁定收回 + 正解，2026-08-13）

**纯像素判据无法区分「空是因为没东西」和「空是因为坏了」**——深色死屏（从未绘制 → 露出
深色背景）= 暗 + 低方差，与「空但健康」深色终端同形。基线表暴露了这个洞（variance 600 vs 586）。

**正解 = 与主机侧事实对账**（回到原始定义）：
> 主机 pane 有内容时，App 终端内容区必须有非背景像素。

- 主机 `capture-pane` 有 N 行内容 → App 空 = **死**
- 主机空 → App 空 = **正常**
- **拿不到主机状态 → INDETERMINATE**（只有历史截图无法对账，不判活不判死）

像素侧 variance/avgGray 降为**辅助**：在「主机有内容」前提下判断屏幕是否真的空。
考卷 R7 已覆盖三态（主机有内容+死屏→死 / 无主机→INDET / 主机空+App空→正常）。

## 六、待补（w-base-v2 模拟器实测）

- 浅色主题的健康 bottomMarginPx 需更多样本（本表只有 2 张浅色）。
- 死屏/空终端的联合判据需在真实场景验证（配主机 capture-pane 对账）。
- P0 事故帧考卷需配套「当时主机有内容」事实才算完整（w-base-v2 已确认 pane=108x87 有内容）。
