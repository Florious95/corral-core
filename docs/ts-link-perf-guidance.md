...
### D2【⚠️ 2026-08-14 关卡1 已证伪触发前提 · 不上线】delta 背压合并（不丢、合帧）

> **订正（w-c1-probe 实测，e2e/harness/c1_sendq_probe_test.go，证据
> `.team/evidence/perf-delta-backpressure-merge.json`）**：sendCh(cap 256) 在真实链路上
> **不会满**——LLM 流式 400–500B/s 下 queue_peak=1（84 帧零缓冲）；最恶劣本地背压
> （对端不读 + 2KB 窗口 + 2000 行突发）queue_peak 也只到 2。机制：内核 socket 缓冲整包
> 吸收突发，writeFrame 从不真正阻塞 → 合并永不触发。**本条按原文预警的「收益量级未实测
> =假说」被关卡 1 判死，不该上线**；实现与红测留档为未来真遇背压时的现成方案。
> **推论**：中间状态可见的真因是慢链把分散到达的 delta 按到达时刻各渲染一次，与服务端
> 队列压力无关 → 减少中间状态的主战场移到 D6（客户端侧合帧）与减往返（D3–D5）。
> 以下原文保留仅作历史记录：
...
### D6【感知层 · 2026-08-14 升为「减少中间状态」主候选】客户端侧「让中间状态不被看见」

- 原与 D2 互为两条路（服务端减少产生 / 客户端减少可见）。**D2 触发前提被关卡 1 证伪后
  （见上），本条升为该目标的主候选**：慢链上 delta 分散到达、按到达时刻各渲染一次
  正是 w-c1-probe 定档的真因，客户端把 <一帧间隔内连续到达的 delta 合成一次渲染
  直接对症。
- **风险不变**：任何客户端渲染缓冲都可能伤打字回显手感（发消息回显是 2 往返理论最优，
  不能再加人为延迟）；v6 时代的「整屏重写抑制」三次证明零收益且一次让 App 全黑——
  合帧窗口必须极小（≤一个 vsync）且有 C1 档机器眼 L4 前后对比才算数。【收益未实测=假说】
