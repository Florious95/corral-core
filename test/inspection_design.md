...
## 〇.5 设计原则（leader 裁定，2026-08-13 事故后补）

> **每一组指标之前，必须先有一条「被测对象还活着吗」的存活判据。**
> 存活判据不过，后续指标一律 BLOCKED 不得 PASS。

依据（本工程实证）：一块永久冻结的空白屏幕，会让所有精细指标算出「正常」——
右缘余量（没内容算不出越界）、底部余量（整屏空）、帧差分（画面不动判 STATIC，像稳定实为死）。
**一个全绿的指标集，测的是一块死掉的屏幕。** 这是「只会亮绿灯的测试层比没有更危险」，
它刚刚真实发生（当前 HEAD 装上模拟器终端内容区整体空白，三条红测+三道回归门+全量单测全绿，
是验收席装 APK 看了一眼才发现的）。

存活判据两条：
1. **`terminalContentAlive`**：主机 pane 有内容时，App 终端内容区必须有非背景像素
   （`contentRatio > 0`，配合主机 `capture-pane` 确认内容存在）。
2. **`screenResponsive`**：主机侧追加一行新内容后，App 端在合理时间内必须出现变化。
   **画面不动 = 红**（刚那个 bug 的决定性症状就是「追加新内容画面仍不变」）。

存活判据不过 → 后续指标一律 **BLOCKED**（屏幕都是死的，别的指标没有意义）。
**这条比单独加指标更重要**：防止指标集在退化状态下集体误报绿。
